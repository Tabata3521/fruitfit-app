...
import { isWorkoutCompleted, WORKOUT_COMPLETION_EVENT } from "../data/workoutCompletion";
...
export default function WorkoutsScreen({ program, selectedWorkoutIndex, onOpenWorkout, onNavigate, access, profile, programAssignment, workoutCycle = {} }) {
  ...
  const restrictionState = programRestrictionState({ profile, course: program.course, programAssignment });

  useEffect(() => {
    const refresh = () => setCompletionRevision((value) => value + 1);
    window.addEventListener(WORKOUT_COMPLETION_EVENT, refresh);
    return () => window.removeEventListener(WORKOUT_COMPLETION_EVENT, refresh);
  }, []);
    ...
    <main className="phone-shell safe-tab-screen">
      <div className="safe-top px-4">
        <header>
          ...
          <p className="mt-2 text-[13px] font-bold leading-5 text-appMuted">{programSummaryTitle(profile, program.course)}</p>
          ...
          <div className="relative mt-3 h-[154px] overflow-hidden rounded-[24px] bg-[#070B07] shadow-card">
            ...
            <div className="relative z-30 flex h-full max-w-[84%] flex-col justify-end p-4 pr-16 text-white">
              ...
              <h2 className="mt-1 line-clamp-4 text-[16px] font-black leading-[1.08]">{programSummaryTitle(profile, program.course)}</h2>
            </div>
          </div>
          <ProgramRestrictions profile={profile} course={program.course} programAssignment={programAssignment} />
        </header>
        ...
        <section className="mt-5 space-y-2">
          {!visibleWorkouts.length && (
            <div className="rounded-[22px] border border-appBorder bg-appCard p-4 text-center">
              ...
              <p className="mt-1 text-[12px] leading-5 text-appMuted">После заявки я свяжусь с тобой по электронной почте.</p>
            </div>
          )}
          {visibleWorkouts.map((workout, index) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => {
                  if (!locked) onOpenWorkout(safeSourceIndex);
                }}
                disabled={locked}

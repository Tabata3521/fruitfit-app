...
export default function HeroWorkoutCard({ program, workout, onStart }) {
  const exerciseCount = workout?.exercises?.length || 0;
  const lessonNumber = workout?.lesson?.lesson_number || 1;
  const totalLessons = program?.workouts?.length || workout?.lessons?.length || 1;
  ...
  return (
    <section className="relative min-h-[244px] overflow-hidden rounded-[26px] bg-[#050805] p-4 text-white shadow-card">
      ...
      <img
        ...
        decoding="async"
        draggable="false"
        className="pointer-events-none absolute bottom-0 right-[-18px] z-20 h-[92%] w-[52%] object-contain object-bottom opacity-95 mix-blend-lighten drop-shadow-[0_18px_30px_rgba(0,0,0,0.62)]"
        style={{
          WebkitMaskImage:
      ...
      />
      <div className="pointer-events-none absolute inset-y-0 right-[28%] z-[25] w-[34%] bg-[linear-gradient(90deg,#050805_0%,rgba(5,8,5,0.78)_38%,rgba(5,8,5,0.24)_76%,transparent_100%)]" />
      ...
      <div className="relative z-30 flex min-h-[212px] max-w-[60%] flex-col pb-[62px]">
        ...
        <p className="mt-2 line-clamp-2 text-[13px] leading-5 text-white/74">{subtitle}</p>
